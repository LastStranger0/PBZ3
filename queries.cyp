...
6. Запрос количества учебных предметов, по которым у студента Щура Антона автомат:
MATCH (n:Student)-[:AVTOMAT]->(m:Subject) WHERE n.name = "Anton" return count(m)

7. Запрос предметов, по которым у студента Тимофея Киневича автомат:
MATCH (n:Student),(m:Subject) WHERE n.name = "Timophei" AND (n)-[:AVTOMAT]->(m) return m
...
9. Запрос всей справочной информации о студенте Андрееве Глебе:
MATCH (n:Student) WHERE n.name = 'Gleb' return n.name,n.last_name, n.age, n.type
